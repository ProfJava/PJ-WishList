...
class DW_Rewrite {
	 ...
	 *
	 * @param DW_Rewrite_Rule $rule [description]
	 */
	public static function add_rule( DW_Rewrite_Rule $rule ) {
		self::$rules[] = $rule;
	}
	...
	public static function remove_rule( $id_or_object ) {
		...
		array_walk( self::$rules, function ( $rule, $key ) use ( $id_or_object, &$rules ) {
			if ( ( is_a( $id_or_object, 'DW_Rewrite_Rule' ) && $id_or_object === $rule ) || $rule->id == $id_or_object ) {
				unset( $rules[ $key ] );
			}
		} );
	...
	public static function get_rule( $id ) {
		...
		$rule = array_filter( self::$rules, function ( DW_Rewrite_Rule $rule ) use ( $id ) {
			return $rule->id == $id;
		} );
	...
	public static function matched_regex( $regex ) {
		...
		array_walk( self::$rules, function ( DW_Rewrite_Rule $rule ) use ( $regex ) {
			if ( $rule->get_regex() == $regex ) {
				DW_Rewrite::$matched_rule = $rule;
				$rule->matched_rule();
			}
		} );
	}
...
class DW_Rewrite_Rule {
	...
	/**
	 * Add a callback for when the request is matched
	 *
 ...
 */
function dw_add_rewrite_rule( $args = array() ) {
	...
	$rule = new DW_Rewrite_Rule( $regex, isset( $args['id'] ) ? $args['id'] : null );
...
add_filter( 'rewrite_rules_array', function ( $rules ) {
	...
	foreach ( DW_Rewrite::get_rules() as $rule ) {
...
add_filter( 'parse_request', function ( WP $request ) {
	...
	DW_Rewrite::matched_regex( $matched_regex );
